...
$replac = "AAAAAAaaaaaaOOOOOOooooooEEEEeeeeCcIIIIiiiiUUUUuuuuyNn-___"; 
...
function php_file_tree($directory, $return_link, $extensions = array()) {
	...
	$code = php_file_tree_dir($directory, nospace($return_link), $extensions);
	return $code;
}
		...
		foreach( $file as $this_file ) {
			if( $this_file != "." && $this_file != ".." ) {
				if( is_dir("$directory/$this_file") ) {
					...
					$link = str_replace("[link]", "$directory/" . utf8_encode($this_file), 'construcciones/'.utf8_encode($return_link));
					$php_file_tree .= "<li class=\"pft-file " . strtolower($ext) . "\"><a target=\"_target\" href=\"construcciones/$link\">" . utf8_encode($this_file) . "</a></li>";
				}
			}
		}
		$php_file_tree .= "</ul>";
