...
$sql = "SELECT * FROM obras_neutralizaciones where estado != '7' order by estado asc";
...
while( $row=$db->fetch_array($query) ) {  // preparing an array
    ...
    if(permiso('admin') || permiso('obras')){
        ...
        <a class="i-dt-list" onclick="editar_neutralizacion('.$row['idneutralizaciones'].')" data-toggle="tooltip" title="Editar datos"><i class="fa fa-edit"></i></a>';  
    }else{
        $nestedData[] = '<a class="i-dt-list" href="obra?id='.$row['idobras'].'" data-toggle="tooltip" title="Ver informe"><i class="fa fa-info-circle"></i></a>';
    }
     ...
     if($row['estado'] == 0 || $row['estado'] == NULL){ 
        ...
        $nestedData[] = '<div id="main"><div class="container"><div class="accordion" id="faq"><div id="faqhead'.$row['idneutralizaciones'].'"><a href="#"  data-toggle="collapse" data-target="#faq'.$row['idneutralizaciones'].'" aria-expanded="true" aria-controls="faq'.$row['idneutralizaciones'].'">Mostrar/Ocultar Observaciones</a></div><div id="faq'.$row['idneutralizaciones'].'" class="collapse" aria-labelledby="faqhead'.$row['idneutralizaciones'].'" data-parent="#faq'.$row['idneutralizaciones'].'"><div class="card-body">'.textarea($row['observaciones']).'</div></div></div>';
         }else{
            $nestedData[] = '';
        }        
