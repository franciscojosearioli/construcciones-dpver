...
$sql = "SELECT * FROM certificados_obras_redeterminados WHERE tipo='provisorio' ORDER BY registro_fecha DESC ";
$query= $db->query($sql);
...
$nestedData[] = '<a href="../uploads/Obras/'.$obra['idobras'].'/Certificaciones/Redeterminados/'.$row['archivo'].'" target="_blank"><i class="fa fa-download"></i>ORIGINAL</a> <a href="../uploads/Obras/'.$obra['idobras'].'/Certificaciones/Redeterminados/'.$row['archivo_copia'].'" target="_blank"><i class="fa fa-download"></i>COPIA</a>';
...
$nestedData[] = '<input type="text" value="'.$row['idcertificados_obras'].'" id="n_certid" hidden><select disabled id="change_estado" onchange="cambiar_estado('.$row['idcertificados_obras'].',this.value);"><option value="0" >Confeccionado</option><option value="1">Emitido</option><option value="2" selected>Aprobado</option></select>';
