...
$items = rows_cotizaciones_obras($obra_id,$obra['idcotizaciones']);
$planoficial =  rows_planes_obras($obra_id,$obra['idplanes_de_trabajo']);
...
<div class="mr-auto my-auto mr-3"><?php if($obra['anticipo_financiero'] == 1 && empty($certificadocero)){ ?>
            <a id="aplica_anticipo" class="btn btn-light" >Confeccion Anticipo financiero</a>
 
<?php } ?>
            <a id="certificado_rp"  class="btn btn-light">Confeccion Certificado RP</a>
</div>
</div>
</div>
  <div class="col-11">
              ...
              <div class="p-r-10 p-l-10">
                <div class="col-lg-12 col-md-12 col-sm-12">
                  <div class="row justify-content-center">
                    <center>
                    <span id="texto_nro_cert">CERTIFICADO Nº:</span> <input type="text" style="width:40px;" name="numero_certificado" id="nro_cert" min="0" required> <span id="leyenda_cer_rp"></span>
    ...
    <div class="row p-10">
        ...
        <div class="row p-10">
            <div class="col-lg-7 col-md-7 col-sm-12">
              <div class="p-r-10 p-l-10">
                <div class="col-lg-12 col-md-12 col-sm-12">
                  <?php if($obra['monto_vigente'] != $obra['contrato_monto']){ ?>
                  <div class="row">
                    IMPORTE DE CONTRATO: <?php echo '$ '.numero($obra['contrato_monto']); ?>
                  </div>
                  <div class="row">
                    IMPORTE APROBADO: $ <?php echo '$ '.numero($obra['monto_vigente']); ?>
                  </div>
                      <?php }else{ ?>
                  <div class="row">
                    IMPORTE DE CONTRATO: <?php echo '$ '.numero($obra['monto_vigente']); ?>
                        ...
                        <div class="row hide" id="importe_contrato_rp">
                    IMPORTE RP: $ <input type="number" name="importe_rp" id="importe_rp" style="width:80%;border:none;"><input type="text" name="if_rp" hidden>
                  </div>
                </div>
              </div>
            </div>
               ...
               <div class="p-r-10 p-l-10">
                <div class="col-lg-12 col-md-12 col-sm-12">
                  <div class="row" id="fecha_medicion_anticipo">
                    FECHA DE MEDICION: <input type="date" name="fecha_medicion" required>
                  </div>
                </div>
              </div>
            </div>  
              ...
              <div class="p-r-10 p-l-10">
                <div class="col-lg-12 col-md-12 col-sm-12">
                  <div class="row justify-content-center">
                    ...
                    <select style="width:85px;" name="month" id="month">
                    ...
                    </select> DE <input type="number" style="width:55px;" name="year" id="year" min="0" value="2021"> 
                    </center>
                  </div>
                </div>
              </div>
...
$u_i = ultimo_item_certificado($ultimo_certificado['idcertificados_obras'],$item['idobras_items']);
    ...
    ?>
      ...
      <td><input type="number" onKeyUp="if(this.value><?php echo ($item['cantidad_aprobada']-$u_i['cantidad_acumulada']); ?>){this.value='<?php echo ($item['cantidad_aprobada']-$u_i['cantidad_acumulada']); ?>';}else if(this.value<0){this.value='0.000';}" name="cantidad[]" style="width:80px;" min="0" step="any" id="valor_nuevo_acumulado" max="<?php if(!empty($ultimo_certificado['idcertificados_obras'])){ echo ($item['cantidad_aprobada']-$u_i['cantidad_acumulada']); } else {echo $item['cantidad_aprobada'];} ?>">
      ...
      <td>
        <input type="number" readonly style="width:70px;border:none" value="<?php if(!empty($u_i)){ echo $u_i['cantidad_acumulada']; }else { echo '0.000'; } ?>" max="<?php echo $item['cantidad_aprobada']; ?>" name="cantidad_acumulada[]">
...
<td style="border:1px solid #000" ><input type="number" name="monto_total_aprobado" id="monto_total_aprobado" readonly style="width:80%;border:none;" value="<?php echo $obra['monto_vigente']; ?>"></td>
...
<td style="border:1px solid #000"><input type="text" name="obra_prevista" style="width:80%;border:none;" id="monto_planoficial"></td>
<td style="border:1px solid #000;width:20%;"><input type="text" style="width:60%;border:none;" id="avance_planoficial" name="avance_planoficial" required> %</td>
</tr>
<tr>
<td style="border:1px solid #000">ATRASO O ADELANTO</td>
<td style="border:1px solid #000"><input type="number" name="atraso_adelanto" id="atraso_adelanto"  style="width:40%;border:none;" >%</td>
...
<td style="border:1px solid #000"><input type="text" name="plazo_transcurrido" style="width:100%;" required></td>
<td style="border:1px solid #000;width:20%;"></td>
</tr>
<!--<tr>
...
<div class="row justify-content-center" id="info_anticipo">
      <div class="col-8">
      <div class="card-body cards-titulo">   
         <center>
            ANTICIPO FINANCIERO
...
<tr>
  ...
  <td><input type="number" style="border:none" name="monto_anticipo_financiero" value="<?php echo ($obra['contrato_monto']*$obra['valor_anticipo_financiero'])/100; ?>" readonly></td>
  <td><input type="number" style="border:none" name="valor_anticipo_financiero" value="<?php echo $obra['valor_anticipo_financiero']; ?>" readonly></td>
</tr>
<?php 
...
<tr id="anticipo_descuento">
  ...
  <td><input type="number" name="monto_descuento_anticipo" id="monto_descuento_anticipo" value="" style="border:none" readonly ><!--resto:<span id="resto_monto_anticipo"></span>--></td>
  <td><input type="number" name="valor_descuento_anticipo" id="valor_descuento_anticipo" value="" style="border:none" readonly ><!--resto:<span id="resto_porcentaje_anticipo"></span>--></td>
</tr>
</tbody>
</table>
</div>
      ...
      <div class="card-body cards-titulo">
        <div class="d-flex flex-wrap">
          <div class="ml-auto my-auto mr-3">
            ...
            <a onclick="confirmar_certificado()" title="Confirmar" data-toggle="tooltip" style="color:green;display:none;" id="confirmar_cer_rp"><i class="fas fa-check"></i> Confirmar</a>
...
$.ajax({
            type: 'POST',
            url: 'includes/ajax/consulta_issetcert.php',
            data: "valor=" + n_cert +"&idobras=" + idobras,
...
swal({
  ...
  .then((willDelete) => {
if (willDelete) {
    swal("No se ha creado el certificado debido a que ya existe.", {
    icon: "warning",
    });
  } else {
    swal("Los cambios permanecen.");
...
var input_cantidades_aprobadas = '';
  ...
  $('#importe_rp').val(suma_importe.toFixed(2));
            //$('#sum_total').html(suma_importe);  
...
$('#certificado_rp').on( 'click', function(){ 
$('#certificado_rp').hide();
$('#aplica_anticipo').hide();
$('#aplica_anticipo').hide();
$('#anticipo_acumulado').hide();
$('#anticipo_descuento').hide();
$('#leyenda_cer_rp').html('R.P.');
$('#info_anticipo').hide();
$('input[name="cantidad[]"]').removeAttr("min");
$('input[name="cantidad[]"]').removeAttr("max");
$('input[name="cantidad[]"]').removeAttr("onKeyUp");
$('input[name="plazo_contractual"]').hide();
$('input[name="plazo_transcurrido"]').hide();
$('input[name="plazo_aprobado"]').hide();
$('input[name="atraso_adelanto"]').hide();
$('input[name="obra_prevista"]').hide();
$('input[name="avance_planoficial"]').hide();
$('#importe_contrato_rp').show();
$('#confirmar_certificado').hide();
$('#confirmar_cer_rp').show();
$('input[name="if_rp"]').val(1);
});
