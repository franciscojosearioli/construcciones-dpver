...
$sql = "SELECT * FROM certificados_obras WHERE estado=1 ORDER BY registro_fecha DESC ";
$query= $db->query($sql);
...
if(!empty($row['numero_resolucion'])){
$nestedData[] = 'N '.$row['numero_resolucion'].' el '.$row['fecha_resolucion'].' <a class="i-dt-list" onclick="change_resolucion('.$row['idcertificados_obras'].')" data-toggle="tooltip" title="Editar"><i class="fas fa-edit"></i></a>';
}else{
   $nestedData[] =  '<center><a class="i-dt-list" onclick="change_resolucion('.$row['idcertificados_obras'].')" data-toggle="tooltip" title="Editar"><i class="fas fa-edit"></i></a></center>';
}

if(permiso('admin') || permiso('certificaciones')){
$nestedData[] = '<input type="text" value="'.$row['idcertificados_obras'].'" id="n_certid" hidden><select id="change_estado" onchange="cambiar_estado('.$row['idcertificados_obras'].',this.value);"><option value="0" >Confeccionado</option><option value="1" selected>Emitido</option><option value="2">Aprobado</option></select>';
}else{

$nestedData[] = '<input type="text" value="'.$row['idcertificados_obras'].'" id="n_certid" hidden><select id="change_estado" disabled onchange="cambiar_estado('.$row['idcertificados_obras'].',this.value);"><option value="0" >Confeccionado</option><option value="1" selected>Emitido</option><option value="2">Aprobado</option></select>';
}
    $data[] = $nestedData;
