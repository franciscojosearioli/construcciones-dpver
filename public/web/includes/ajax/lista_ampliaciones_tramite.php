...
$sql = "SELECT * FROM obras_ampliaciones where estado != '7' order by estado asc";
...
while( $row=$db->fetch_array($query) ) {  // preparing an array
    ...
    if(permiso('admin') || permiso('obras')){
        ...
        <a class="i-dt-list" onclick="editar_ampliacion('.$row['idampliaciones'].')" data-toggle="tooltip" title="Editar datos"><i class="fa fa-edit"></i></a>';
    }else{
        $nestedData[] = '<a class="i-dt-list" href="obra?id='.$row['idobras'].'" data-toggle="tooltip" title="Ver informe"><i class="fa fa-info-circle"></i></a>';
    }
     ...
     if($row['estado'] == 0 || $row['estado'] == NULL){ 
        ...
        $nestedData[] = '<div id="main"><div class="container"><div class="accordion" id="faq"><div id="faqhead'.$row['idampliaciones'].'"><a href="#"  data-toggle="collapse" data-target="#faq'.$row['idampliaciones'].'" aria-expanded="true" aria-controls="faq'.$row['idampliaciones'].'">Mostrar/Ocultar Observaciones</a></div><div id="faq'.$row['idampliaciones'].'" class="collapse" aria-labelledby="faqhead'.$row['idampliaciones'].'" data-parent="#faq'.$row['idampliaciones'].'"><div class="card-body">'.textarea($row['observaciones']).'</div></div></div>';
         }else{
            $nestedData[] = '';
        }        
