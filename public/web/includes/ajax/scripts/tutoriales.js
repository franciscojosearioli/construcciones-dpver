...
function form_agregar(flag)
{
	if (flag)
	{
		$("#listar_tutoriales").hide();
		$("#ver_form_agregar").show();
		$("#btn_agregar").hide();
	}
	else
	{
		$("#listar_tutoriales").show();
...
$('#editar_usuarios').html(respuesta).show();
  ...
  $(document).ready(function() {
    $('#tabla_tutoriales').DataTable( {

dom: 'lBfrtip',        
ajax: {
          url : 'includes/ajax/listar_tutoriales.php',
          type: 'POST'
        },
        buttons: [
        {
    ...
    $('#tabla_tutoriales_processing').hide();
} );

$('.videos-tutoriales').owlCarousel({
  loop:true,
  margin:10,
