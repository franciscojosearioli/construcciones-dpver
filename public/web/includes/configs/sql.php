...
function buscar_obras_globales($key){
  global $db;
    $sql  =" SELECT *";
    $sql  .=" FROM obras";
    $sql  .=" WHERE where nombre LIKE '%{$key}%' OR expediente LIKE '%{$key}%' OR contratista LIKE '%{$key}%' OR ubicacion LIKE '%{$key}%' OR descripcion LIKE '%{$key}%' OR expediente LIKE '%{$key}%'";
    return find_by_sql($sql);
}
?>
